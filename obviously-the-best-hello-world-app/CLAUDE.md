# CLAUDE.md — obviously-the-best-hello-world-app
...
## Skills
The `.claude/skills/` directory contains reusable development skills:

| Skill | Description | Invoke |
|-------|-------------|--------|
| `coding-standards` | Naming, imports, type safety, style rules | Auto |
| `verification-loop` | Post-change quality gate | `/verification-loop` |
| `tdd-workflow` | Test-driven development process | Auto |
| `add-endpoint` | Checklist for new API endpoints | `/add-endpoint GET /path` |
| `add-migration` | Create DB migrations | `/add-migration description` |
| `db-schema` | Database schema reference | Auto |
| `project-structure` | File tree and layout conventions | Auto |
| `docker-patterns` | DooD, compose, container lifecycle | Auto |
